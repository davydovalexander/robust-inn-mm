'''
Code is adapted from the implicit deep learning repository on GitHub
https://github.com/beeperman/idl
'''
...
LOGDIR = osp.join(os.path.dirname(__file__), "logs")
...
class Logger(object):
    ...
    def __init__(self, printstr=[], save_every=1, dir_name=None, dir_path_with_time=True):
        ...
        if dir_name is None:
            dir_name = datetime.datetime.now().strftime("LogRun_%Y-%m-%d_%H_%M_%S")
        else:
            time = datetime.datetime.now().strftime("_%Y-%m-%d_%H_%M_%S") if dir_path_with_time else ""
